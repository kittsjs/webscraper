/**
 * Wforwomen API Service
 * Handles API calls to Wforwomen product API
 */

class WforwomenApiService {
  /**
  ...
  /**
   * Fetches product data from Wforwomen API
   * @param {string} url - Product page URL
  ...
  async fetchProduct(url) {
    try {
      ...
      console.log('Fetching Wforwomen product from API:', apiUrl);
      ...
      if (!response.ok) {
        throw new Error(`Wforwomen API request failed: ${response.status} ${response.statusText}`);
      }

      const data = await response.json();
      return data;
    } catch (error) {
      console.error('Error fetching product from Wforwomen API:', error.message);
      return null;
    }
  }
   ...
   */
  extractWforwomenGalleryImages(apiResponse) {
    try {
      ...
      console.error('Error extracting Wforwomen gallery images from API response:', error.message);
      return [];
    }
  }

  /**
   * Gets product image from Wforwomen product page URL
   * @param {string} url - Product page URL
  ...
  async getProductImage(url) {
    try {
      ...
      const imageList = this.extractWforwomenGalleryImages(apiResponse);
...
export default new WforwomenApiService();
